...
# Do not allow uninitialised variables
set -o nounset
...
# ×º°”˜`”°º×` ₪  Folder paths  ₪ ×º°”˜`”°º×`
DIR_SCRIPT=`dirname $0`
DIR_CURRENT=`pwd`
#echo $DIR_CURRENT $DIR_SCRIPT
# -----------------------------------------
...
ks_main() {
  ...
  # Meta-programming scripts and functions
  ks_meta
...
# Meta-programming small shell scripts in bin/ path
[[ -d bin ]] || mkdir bin/

# Using here-doc to write snippets of useful scripts to files
file=bin/fn.sh
[[ -f $file ]] || cat << 'EOF' > $file
#!/usr/bin/env bash
